...
<html>
  ...
  <body>
    <%
      ...
      ResultSet listado = s.executeQuery ("SELECT COUNT(*) FROM monstruo WHERE nomMon LIKE '%" + request.getParameter("nomMon") + "%'");
      listado.next();
      ...
      if (coincidencias == 0) {
        out.println("<script>document.location = \"bestiario.jsp\"</script>");
      }

      ResultSet listado2 = s.executeQuery ("SELECT * FROM monstruo WHERE nomMon LIKE '%" + request.getParameter("nomMon") + "%'");

      // Si hay un único cliente con el nombre especificado, vamos directamente
      // a la página de detalle.
      if (coincidencias == 1) {
        listado2.next();
        out.println("<script>document.location = \"detalleMon.jsp?idMon=" + listado2.getInt("idMon") + "\"</script>");
      }
              ...
              <div class="col s6 m6 l6">
                <div class="card grey lighten-5">
                  ...
                  while (listado2.next()) {
                    %>
                    <tr>
                      <td><%=listado2.getString("nomMon") %></td>
                      <form method="post" action="detalleMon.jsp">
                        <input type="hidden" name="idMon" value="<%=listado2.getString("idMon") %>">
                        <td>
